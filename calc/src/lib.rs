...
impl Calculator {
    ...
    pub fn save_code_to_file(&self, base_path: &str) -> std::io::Result<()> {
        let code = self.generate_calc_code();
        let file = File::create(&format!("{}/calc_code.rs", base_path))?;
    ...
    /// 生成计算代码
    pub fn generate_calc_code(&self) -> String {
        ...
        // lfwx (Low Pressure Feedwater Heaters)
        code.push_str("\tprintln!(\"\\n  --- 低压给水加热器 (lfwx) ---\");\n");
        ...
        // // hfwx (High Pressure Feedwater Heaters)
        code.push_str("\tprintln!(\"\\n  --- 高压给水加热器 (hfwx) ---\");\n");
